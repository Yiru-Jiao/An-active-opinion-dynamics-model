...
to-report similarity_effect ;; generate interactive willingness based on Similarity Effect
  let decision (ifelse-value
    (first [faction] of turtle i = 0 or first [faction] of turtle j = 0) [
      (ifelse-value
        (random-float 1 <= (0.5 + 13 * (atan (10 * rejection-of-different-opinions) 1) / 180 / 30) ) [true]
        [false]
        )
    ]
    (first [faction] of turtle i = first [faction] of turtle j) [
      (ifelse-value
        (random-float 1 <= (0.8 + (atan (10 * rejection-of-different-opinions) 1) / 180 / 5) ) [true]
        [false]
        )
    ] [
      (ifelse-value
        (random-float 1 <= (0.2 + (atan (10 * rejection-of-different-opinions) 1) / 180 / 2) ) [true]
        [false]
        )
    ]
    )
